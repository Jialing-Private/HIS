<template>
    <div class="fater-body-show">
			...
			<div>
				<el-table v-loading="loading"  :id="departmentsTable" stripe
					...
					element-loading-background="rgba(124, 124, 124, 0.8)" :data="pageInfos" border>
						...
						<el-table-column align="center" fixed="right" label="操作处理" width="150px">
		...
		<el-dialog title="添加信息" width="600px" :visible.sync="showAddFlag">
			<el-form label-width="90px" :model="departmentsForm">
				<el-form-item label="科室编号">
					<el-input v-model="departmentsForm.did" 
						placeholder="请输入科室编号…" autocomplete="off"></el-input>
				</el-form-item>
				<el-form-item label="科室名称">
					<el-input v-model="departmentsForm.name" 
						placeholder="请输入科室名称…" autocomplete="off"></el-input>
				</el-form-item>
				<el-form-item label="科室类型">
					<el-select v-model="departmentsForm.status" placeholder="选择科室类型…" clearable>
						<el-option v-for="item in statusList" :label="item.label" :value="item.status" :key="item.status"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="科室描述">
					<el-input type="textarea" v-model="departmentsForm.describe" 
